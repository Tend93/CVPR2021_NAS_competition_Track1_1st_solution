    ...
    def copy_weight2twin(self, requires_grad=True):
        if self.use_branch:
            for i in range(self.b_num):
                ...
                #     conv_layer[0].weight.requires_grad = False
                #     conv_layer[1].weight.requires_grad = False
                #     conv_layer[1].bias.requires_grad = False
...
class ResNet(nn.Module):
    def __init__(self, blocks, len_list, use_branch=[True, True, True, True], 
        ...
        self.layer2 = self.make_layer(self.len_list, block=blocks[1], block_type=self.block, stride=2, use_branch=use_branch[2], b_num=4)
        self.layer3 = self.make_layer(self.len_list, block=blocks[2], block_type=self.block, stride=2, use_branch=use_branch[3], b_num=4)
