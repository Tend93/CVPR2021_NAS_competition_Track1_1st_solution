...
parser.add_argument('--FT', type=str2bool, default=False, help='just test the model')
...
def train(train_queue, model, optimizer, args, logging, criterion):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        for sample_i in range(args.sample_num):  
            ...
            loss = (1 - distill_coeff) * loss + distill_coeff * torch.mean(torch.sum(- soft_label * nn.LogSoftmax(dim=1)(st_outputs), 1))
            loss.backward()
